...
func InitRouter(g *gin.Engine) *gin.Engine {
	...
	v1 := g.Group(config.App.BaseURL)
	{
		...
		v1.GET("/associations/random/forward", responseWrapper(association.GetRandomWordForwardAssociations))
		v1.GET("/associations/random/backward", responseWrapper(association.GetRandomWordBackwardAssociations))
	}
